        ...
        {tab==="estoque"&&<Card t={t}><ST icon="📦">Estoque</ST>
          ...
          {stkTab==="entrada"&&<>
            ...
            <div style={{marginTop:"16px",padding:"12px",background:t.sectionBg,borderRadius:"8px",border:`1px solid ${t.cardBorder}`}}>
              ...
              <input type="file" accept=".xml,.pdf" onChange={async(e)=>{const file=e.target.files?.[0];if(!file)return;const fname=file.name.toLowerCase();setFbMsg("Lendo arquivo...");try{let items=[];if(fname.endsWith(".xml")){const txt=await file.text();const parser=new DOMParser();const xml=parser.parseFromString(txt,"text/xml");const dets=xml.getElementsByTagName("det");for(let i=0;i<dets.length;i++){const det=dets[i];const xProd=(det.getElementsByTagName("xProd")[0]?.textContent||"").toUpperCase();const qCom=parseFloat(det.getElementsByTagName("qCom")[0]?.textContent||"0");const vUnCom=parseFloat(det.getElementsByTagName("vUnCom")[0]?.textContent||"0");const uCom=(det.getElementsByTagName("uCom")[0]?.textContent||"").toUpperCase();items.push({desc:xProd,qty:qCom,cost:vUnCom,unit:uCom})}}else if(fname.endsWith(".pdf")){if(!window.pdfjsLib){const sc=document.createElement("script");sc.src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js";document.head.appendChild(sc);await new Promise(r=>{sc.onload=r;setTimeout(r,5000)})}if(window.pdfjsLib){window.pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js";const ab=await file.arrayBuffer();const pdf=await window.pdfjsLib.getDocument({data:ab}).promise;let ft="";for(let pg=1;pg<=pdf.numPages;pg++){const page=await pdf.getPage(pg);const tc=await page.getTextContent();ft+=tc.items.map(z=>z.str).join(" ")+"\n"}const rows=ft.split("\n").filter(r=>r.trim());rows.forEach(row=>{const upper=row.toUpperCase();if(upper.includes("VINIL")||upper.includes("LONA")||upper.includes("MANTA")||upper.includes("FILTRO")||upper.includes("REFLETOR")||upper.includes("SKIMMER")){const nums=row.match(/[\d]+[.,]?[\d]*/g);if(nums&&nums.length>=2){const qty=parseFloat(nums[nums.length-3]?.replace(",",".")||nums[0].replace(",","."))||0;const cost=parseFloat(nums[nums.length-2]?.replace(",",".")||"0")||0;items.push({desc:upper,qty,cost,unit:""})}}})}else{setFbMsg("Erro ao carregar leitor PDF")}}const entries=[];items.forEach(it=>{const d=it.desc;let matched=null;const is07=d.includes("0,7")||d.includes("0.7");const is08=d.includes("0,8")||d.includes("0.8");CAT.forEach(p=>{const pn=p.n.toUpperCase();const words=pn.split(" ").filter(w=>w.length>3);const mainWords=words.slice(0,2);if(mainWords.every(w=>d.includes(w))){matched=p}else if(d.includes("LONA AQUATICA")&&pn.includes("LONA AQUATICA")){matched=p}});if(!matched){const stampNames=CAT.filter(p=>p.c.startsWith("Vinil")).map(p=>({id:p.id,name:p.n.replace(/ 0,[78]mm/,"").toUpperCase()}));for(const sn of stampNames){const snWords=sn.name.split(" ").filter(w=>w.length>3);if(snWords.length>0&&snWords.every(w=>d.includes(w))){const suffix=is07?"7":is08?"8":"7";const baseId=sn.id.replace(/[78]$/,"");matched=CAT.find(p=>p.id===baseId+suffix);break}}}entries.push({catId:matched?.id||"",qty:String(it.qty),cost:String(it.cost),_desc:it.desc})});if(entries.length>0){setEntItems(entries);setFbMsg(entries.length+" itens encontrados! Confira e clique Dar Entrada")}else{setFbMsg("Nenhum item encontrado no arquivo")}setTimeout(()=>setFbMsg(""),6000)}catch(err){console.error(err);setFbMsg("Erro: "+(err.message||err));setTimeout(()=>setFbMsg(""),4000)}e.target.value=""}} style={{fontSize:"11px",color:t.text}}/>
            </div></>}
          {stkTab==="historico"&&<>
