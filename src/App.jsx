...
const CAT=[
  ...
  {id:"v07",c:"Vinil",n:"Vinil Acqualiner 0,7mm",s:"Bobina 1,40m",p:45,un:"m2"},{id:"v08",c:"Vinil",n:"Vinil Acqualiner 0,8mm",s:"Bobina 1,40m",p:55,un:"m2"},
  {id:"sold",c:"Vinil",n:"Solda Vinil (cola)",s:"Frasco 500ml",p:35,un:"un"},{id:"cantn",c:"Vinil",n:"Cantoneira PVC",s:"Barra 3m",p:18,un:"un"},
  {id:"tubo50",c:"Hidraulica",n:"Tubo PVC 50mm",s:"Barra 6m",p:32,un:"un"},{id:"tubo60",c:"Hidraulica",n:"Tubo PVC 60mm",s:"Barra 6m",p:45,un:"un"},
  {id:"cola",c:"Hidraulica",n:"Cola PVC",s:"Frasco 175g",p:12,un:"un"},{id:"veda",c:"Hidraulica",n:"Veda Rosca",s:"Rolo 18mm",p:5,un:"un"},
  {id:"areia",c:"Filtros",n:"Areia p/ Filtro",s:"Saco 25kg",p:35,un:"un"},{id:"bomba",c:"Equipamentos",n:"Motobomba 1/3CV",s:"Dancor/Weg",p:450,un:"un"},
];
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"3px",maxHeight:"400px",overflow:"auto"}}>
              {CAT.filter(p=>!stkFilter||p.n.toLowerCase().includes(stkFilter.toLowerCase())||p.c.toLowerCase().includes(stkFilter.toLowerCase())).map(p=>{const s=stk[p.id]||{qty:0,minQty:2,lastCost:p.p};const low=s.qty>0&&s.qty<=s.minQty;const zero=s.qty<=0;return <div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",background:zero?"#fef2f2":low?"#fffbeb":t.sectionBg,borderRadius:"6px",border:`1px solid ${zero?"#fecaca":low?"#fde68a":t.cardBorder}`}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"6px"}}><div style={{fontSize:"16px",fontWeight:"800",color:zero?"#dc2626":low?"#f59e0b":"#16a34a",minWidth:"35px",textAlign:"right"}}>{s.qty}</div><span style={{fontSize:"8px",color:t.textMuted}}>{p.un}</span>
                  <div style={{display:"flex",gap:"2px"}}><button onClick={()=>{const ns={...stk};if(!ns[p.id])ns[p.id]={qty:0,minQty:2,lastCost:p.p};ns[p.id].qty++;saveStk(ns)}} style={{width:"20px",height:"20px",borderRadius:"4px",border:"none",background:"#16a34a",color:"#fff",fontSize:"11px",cursor:"pointer",fontWeight:"700"}}>+</button><button onClick={()=>{const ns={...stk};if(!ns[p.id])ns[p.id]={qty:0,minQty:2,lastCost:p.p};ns[p.id].qty=Math.max(0,ns[p.id].qty-1);saveStk(ns)}} style={{width:"20px",height:"20px",borderRadius:"4px",border:"none",background:"#dc2626",color:"#fff",fontSize:"11px",cursor:"pointer",fontWeight:"700"}}>-</button></div>
                </div></div>})}
            </div></>}
          {stkTab==="entrada"&&<>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
              {entItems.map((ei,idx)=><div key={idx} style={{display:"flex",gap:"5px",alignItems:"center"}}>
                <select value={ei.catId} onChange={e=>{const n=[...entItems];n[idx].catId=e.target.value;setEntItems(n)}} style={{flex:2,padding:"5px",border:`1px solid ${t.cardBorder}`,borderRadius:"5px",fontSize:"10px",background:t.inputBg,color:t.text}}><option value="">Selecionar...</option>{CAT.map(p=><option key={p.id} value={p.id}>{p.c} - {p.n}</option>)}</select>
            ...
            <div style={{marginTop:"16px",padding:"12px",background:t.sectionBg,borderRadius:"8px",border:`1px solid ${t.cardBorder}`}}>
              ...
              <input type="file" accept=".xml,.pdf" onChange={async(e)=>{const file=e.target.files?.[0];if(!file)return;const fname=file.name.toLowerCase();try{if(fname.endsWith(".xml")){const txt=await file.text();const parser=new DOMParser();const xml=parser.parseFromString(txt,"text/xml");const dets=xml.getElementsByTagName("det");const nfEl=xml.getElementsByTagName("nNF");const nfNum=nfEl.length>0?nfEl[0].textContent:"NF";const entries=[];for(let i=0;i<dets.length;i++){const det=dets[i];const prods=det.getElementsByTagName("xProd");const qts=det.getElementsByTagName("qCom");const vals=det.getElementsByTagName("vUnCom");if(prods.length>0){const xProd=prods[0].textContent||"";const qCom=qts.length>0?parseFloat(qts[0].textContent)||1:1;const vUn=vals.length>0?parseFloat(vals[0].textContent)||0:0;const xl=xProd.toLowerCase();const match=CAT.find(p=>{const w=p.n.toLowerCase().split(" ");return w.some(ww=>ww.length>3&&xl.includes(ww))});if(match)entries.push({catId:match.id,qty:String(qCom),cost:String(vUn)});else entries.push({catId:"",qty:String(qCom),cost:String(vUn),_name:xProd})}}if(entries.length>0){const matched=entries.filter(x=>x.catId);const unmatched=entries.filter(x=>!x.catId);setEntItems(matched.length>0?matched:[{catId:"",qty:"",cost:""}]);setFbMsg("XML lido! "+matched.length+" reconhecidos"+(unmatched.length>0?", "+unmatched.length+" nao reconhecidos":""))}else{setFbMsg("Nenhum item no XML")}}else if(fname.endsWith(".pdf")){setFbMsg("Lendo PDF...");const arrBuf=await file.arrayBuffer();if(!window.pdfjsLib){const sc=document.createElement("script");sc.src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js";document.head.appendChild(sc);await new Promise((r)=>{sc.onload=r;setTimeout(r,5000)})}if(window.pdfjsLib){window.pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js";const pdf2=await window.pdfjsLib.getDocument({data:arrBuf}).promise;let ft="";for(let pg=1;pg<=pdf2.numPages;pg++){const page=await pdf2.getPage(pg);const tc=await page.getTextContent();ft+=tc.items.map(z=>z.str).join(" ")+" "}const fl=ft.toLowerCase();const ent3=[];CAT.forEach(p=>{const ws=p.n.toLowerCase().split(" ");const mw=ws.find(w=>w.length>3)||ws[0];if(fl.includes(mw)&&!ent3.find(x=>x.catId===p.id)){ent3.push({catId:p.id,qty:"1",cost:String(p.p)})}});if(ent3.length>0){setEntItems(ent3);setFbMsg("PDF: "+ent3.length+" itens encontrados. Confira e clique Dar Entrada")}else{setFbMsg("Nenhum produto reconhecido no PDF")}}else{setFbMsg("Erro ao carregar leitor PDF")}}else{setFbMsg("Formato nao suportado")}}catch(err){setFbMsg("Erro: "+(err.message||err))}setTimeout(()=>setFbMsg(""),5000);e.target.value=""}} style={{fontSize:"11px",color:t.text}}/>
            </div></>}
          {stkTab==="historico"&&<>
