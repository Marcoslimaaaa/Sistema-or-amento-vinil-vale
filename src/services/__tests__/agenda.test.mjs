...
import {
  ...
  agruparPorDia, rotuloData, outrosNaRegiao, resumo, emAberto, buscarClientes,
} from "../agenda.js";
...
// ── busca de cliente (a lista passa de 140; sem isso ninguem acha ninguem) ──
const clientes = [
  { id: 1, status: "lead", cN: "Mariana Souza", cC: "Registro-Sp" },
  { id: 2, status: "negociacao", cN: "Ana Paula", cC: "Cajati-Sp" },
  { id: 3, status: "orcamento", cN: "José Carlos", cC: "Peruíbe-Sp" },
  { id: 4, status: "fechou", cN: "Solange", cC: "Peruibe-Sp" },
  { id: 5, status: "perdido", cN: "Ana Perdida", cC: "Registro-Sp" },
  { id: 6, status: "concluido", cN: "Ana Concluida", cC: "Registro-Sp" },
];
check("quem COMECA com o termo vem antes",
  buscarClientes(clientes, "ana").map((q) => q.cN), ["Ana Paula", "Mariana Souza"]);
check("busca sem acento acha o nome com acento",
  buscarClientes(clientes, "jose").map((q) => q.id), [3]);
check("busca por CIDADE tambem funciona, com as duas grafias",
  buscarClientes(clientes, "peruibe").map((q) => q.id).sort(), [3, 4]);
check("perdido e concluido ficam fora", buscarClientes(clientes, "ana").some((q) => q.id === 5 || q.id === 6), false);
check("sem termo, mostra os mais avancados primeiro",
  // fechou(4) > negociacao(3) > orcamento(2): a ordem e 4, 2, 3.
  buscarClientes(clientes, "").map((q) => q.id).slice(0, 2), [4, 2]);
check("termo sem resultado devolve vazio", buscarClientes(clientes, "xyz"), []);
check("respeita o limite", buscarClientes(clientes, "", 2).length, 2);
check("hist vazio nao quebra", buscarClientes(undefined, "ana"), []);
